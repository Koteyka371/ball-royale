...
def test_blizzard_slows_speed():
    ...
    assert abs(ball.speed - 90.0) < 0.1 or abs(ball.speed - 50.0) < 0.1
    assert abs(getattr(ball, 'base_speed', 100.0) - 180.0) < 0.1 or abs(getattr(ball, 'base_speed', 100.0) - 100.0) < 0.1
...
def test_sandstorm_reduces_perception():
    ...
    assert abs(ball.speed - 180.0) < 0.1 or abs(ball.speed - 100.0) < 0.1
