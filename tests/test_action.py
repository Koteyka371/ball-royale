...
class MockEnemy:
    def __init__(self, x=100, y=50, radius=10, ball_type="enemy_ball", alive=True):
        self.x = x
        self.y = y
        self.radius = radius
...
class MockAlly:
    def __init__(self, x=70, y=50, radius=10, ball_type="mock_ball", alive=True):
        self.x = x
        self.y = y
        self.radius = radius
        self.ball_type = ball_type
...
class MockBooster:
    def __init__(self, x=100, y=50, active=True):
        self.x = x
        self.y = y
        self.active = active
...
def test_execute_use_skill():
    ...
    # execute() also calls _update_skill_timer, which reduces timer by 0.1
    assert abs(ball.skill_timer - 4.9) < 0.001
...
def test_obstacle_avoidance():
    # Ball is at 50, 50. Enemy is at 100, 50. Ally is at 70, 50 (directly in between).
    # Ally should apply a repulsive force pushing the ball upwards or downwards.
    ball = MockBall(x=50, y=50)
    world = MockWorld()
    world.enemies = [MockEnemy(x=100, y=50)]
    # Place ally slightly offset on y so repulsion pushes clearly
    world.allies = [MockAlly(x=70, y=49)]
    action_layer = Action(ball, world)

    action_layer.execute("attack", 0.1)

    # We are attacking the enemy at x=100, so we want to move right (x increases).
    # But ally is at y=49, which means edy = 50 - 49 = 1.
    # The repulsive force pushes away from the ally. The ball is at y=50, ally at 49.
    # Repulsion y = edy / dist = 1 / dist. Force pushes in +y direction.
    # So ball should move in +y direction (y > 50).
    assert ball.x > 50 # Moves towards enemy
    assert ball.y > 50 # Pushed upwards away from ally

def test_use_skill_cooldown_reset():
    ball = MockBall()
    ...
    action_layer.execute("use skill", 0.1)
    # Skill was on cooldown, so it shouldn't have been used.
    # But execute() also calls _update_skill_timer, which reduces timer by 0.1
    assert not ball.used_skill
    assert abs(ball.skill_timer - 1.9) < 0.001

    # Force timer to 0 and test
