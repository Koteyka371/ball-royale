...
def test_finals_1v1_arena_generation():
    arena = Finals1v1Arena(arena_size=2000.0, seed=42)
    assert len(arena.rooms) == 3
    assert len(arena.corridors) == 2
    assert len(arena.hazards) == 4

def test_finals_1v1_arena_is_point_inside():
    arena = Finals1v1Arena(arena_size=2000.0, seed=42)
    ...
    assert arena.is_point_inside(150, cy, 10.0)

    # Point inside the right room
    assert arena.is_point_inside(w - 150, cy, 10.0)
