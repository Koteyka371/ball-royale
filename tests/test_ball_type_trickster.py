"""
Auto-generated tests for: Trickster
"""
...
def test_trickster_initialization():
    ball = Trickster(ball_id=1, x=100, y=200)
    assert ball.id == 1
    assert ball.hp == 85
...
def test_trickster_hp_percent():
    ball = Trickster(ball_id=1)
    ball.hp = 85 / 2
    assert abs(ball.get_hp_percent() - 0.5) < 0.01


def test_trickster_take_damage():
    ball = Trickster(ball_id=1)
    ball.take_damage(10)
    assert ball.hp == 85 - 10
    assert ball.alive is True
    ball.take_damage(85)
    assert ball.alive is False


def test_trickster_skill():
    ball = Trickster(ball_id=1)
