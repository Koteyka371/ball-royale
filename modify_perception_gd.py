...
if "has_sonar =" not in content:
    content = re.sub(
        r'(var perception_radius = 300\.0\n\s*if "perception_radius" in self\.ball:\n\s*perception_radius = float\(self\.ball\.perception_radius\)\n)',
        r'\1' + sonar_block,
        content
    )

if "if has_sonar: return false" not in content:
    content = re.sub(
        r'(var intersects_smoke = func\(ent\):\n)',
        r'\1        if has_sonar: return false\n',
        content
    )

# Bypass enemies checking
if "if has_sonar:" not in content.split("var entities = ")[1]:
    content = re.sub(
