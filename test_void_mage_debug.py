...
action._get_enemies = MagicMock(return_value=[enemy])
action.execute("use_skill", 1.0)

bh = world.arena.hazards[0]
print("Created bh at", bh.y, "vx, vy:", getattr(bh, "vx", None), getattr(bh, "vy", None), getattr(bh, "last_updated_tick", None))

world.tick = 2
action._process_physics = lambda delta: None
# attack will call process_hazards
action.execute("attack", 1.0)
print("After execute attack bh.y =", bh.y, "vx, vy:", getattr(bh, "vx", None), getattr(bh, "vy", None), getattr(bh, "last_updated_tick", None))

world.tick = 3
# flee will call process_hazards
action.execute("flee", 1.0)
print("After execute flee bh.y =", bh.y, "vx, vy:", getattr(bh, "vx", None), getattr(bh, "vy", None), getattr(bh, "last_updated_tick", None))
