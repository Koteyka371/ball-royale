...
world = MockWorld()
ball_enemy = MockBall(100, 100, "team_a")
ball_ally = MockBall(50, 100, "team_b")
hazard = MockHazard("team_b", 0, 0, "vampiric_puddle")
world.arena.hazards = [hazard]
world.balls = [ball_enemy, ball_ally]

action1 = Action(ball_enemy, world)
action1.execute('idle', 1.0)
print(f"Enemy HP: {ball_enemy.hp}")

action2 = Action(ball_ally, world)
action2.execute('idle', 1.0)
print(f"Ally HP: {ball_ally.hp}")
