...
def invoke_jules(task_id, area, prompt, branch_name, token):
    ...
    full_prompt = (
        ...
        f"11. IDEAS INBOX: After finishing your code, you MUST invent at least 2 new features/ideas for the game and write them as separate valid JSON files in the 'ideas/' directory (e.g. 'ideas/idea_{task_id}_1.json' and 'ideas/idea_{task_id}_2.json', using unique filenames to prevent overwrite). Format for each file: {{\"title\": \"...\", \"description\": \"...\"}}\n"
        f"12. PRIMARY FOCUS — MECHANICS POLISH & BUGFIXING: Prioritize final mechanics refinement, bugfixes, performance optimization, lag elimination, and physics smoothness.\n"
        f"13. PHYSICAL OBSERVABILITY & VISUAL INTEGRITY: Run physical observation simulations ('python scripts/test_physical_modes.py --fast'). Ensure zero motion jitter, zero vibration spikes, zero ball sticking, and zero simulation deadlocks.\n"
        f"14. HIGH QUALITY & TEST COVERAGE: Write pristine, resilient Python code with robust Pytest assertions in tests/.\n"
    )
