...
class ProceduralArena:
    ...
    def generate(self):
        ...
        for i in range(num_hazards):
            kind = random.choice(["spikes", "lava", "fake_booster", "decoy_item", "link_booster", "stamina_booster", "weather_booster", "poison_cloud", "proximity_trap", "spinning_laser", "healing_spring", "temporal_rift", "bumper", "tornado", "hidden_trap", "silence_booster", "switch", "magnet", "quicksand", "magnet_booster", "vision_booster"])
            if kind == "switch":
                radius = 20.0
                damage = 0.0
            if kind == "spikes":
