...
def test_hazard_generation():
    ...
    for hazard in arena.hazards:
        assert hazard.kind in ["spikes", "lava", "fake_booster", "poison_cloud", "proximity_trap", "explosive_barrel", "healing_spring", "trap", "spinning_laser", "laser_wall", "meteor", "conveyor_belt", "gravity_well", "black_hole", "portal", "teleporter"]
        assert hazard.radius > 0
        assert hazard.damage > 0 or hazard.kind == "healing_spring"
