            ...
            if "kind" in h and h.kind == "flare":
                ...
                if h.has_meta("duration"):
                    ...
                    if dur <= 0:
                        ...
                        var fire_zone_id = 7000 + hazards.size() + new_craters.size() + (randi() % 1000)
                        var fire_zone = ProceduralArenaScript.Hazard.new(fire_zone_id, h.x, h.y, h.radius * 1.5, "fire_zone", 20.0)
