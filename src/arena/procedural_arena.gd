...
func generate():
    ...
    for i in range(num_hazards):
        ...
        if r < 0.05:
            ...
            kind = "damage_reflection_booster"
        elif r < 0.99999:
            kind = "friendly_fire_reflect_booster"
        elif r < 0.999995:
            kind = "mirage_safe_zone"
        elif r < 0.999996:
            kind = "aura_overcharge_booster"
        else:
            kind = "switch"
        ...
        if kind == "whirlpool":
            ...
            if rng.randf() < 0.4:
                var powerups = ["heal", "speed", "shield", "stamina", "cursed", "heat_shield", "cryo_heal"]
    ...
    if current_tick != last_tick:
        ...
        for hazard in hazards:
            ...
            for hazard in hazards:
                if hazard.kind == "ice_patch" or hazard.kind == "ice_patches":
                    ...
                    var active_heatwave = false
                    if "is_heatwave" in self:
                        active_heatwave = self.is_heatwave

                    var active_snowing = false
                    if "is_snowing" in self:
                        active_snowing = self.is_snowing

                    if active_heatwave or temperature > 0.0:
                        var eff_temp = max(temperature, 20.0) if active_heatwave else temperature
                        h_dur -= eff_temp * 0.1 * delta
                        if h_dur <= 0.0:
                            if hazard.has_method("set_meta"):
                                hazard.set_meta("active", false)
                            else:
                                hazard.active = false
                    elif active_snowing or temperature < 0.0:
                        var eff_temp = min(temperature, -20.0) if active_snowing else temperature
                        h_dur -= -eff_temp * 0.1 * delta
                        if h_dur <= 0.0:
        ...
        if safe_zone_radius > 0.0:
            ...
            if current_tick % 300 == 0:
                ...
                var item_kinds = ["deployable_slow_bomb", "healing_spring", "damage_link", "emp_burst", "nemesis_booster", "stamina_booster", "vision_booster", "reverse_gravity_booster", "grapple_booster", "material_magnet_booster", "shrink_beam_attachment", "mirage_booster", "heroism_booster", "echolocation_booster", "orbital_emp_strike_item", "sticky_bomb_booster", "fire_sticky_bomb_booster", "floating_debris", "phantom_artifact_item", "blood_pact_artifact_item", "aura_overcharge_booster"]
            ...
            if "kind" in h and h.kind == "flare":
                ...
                if h.has_meta("duration"):
                    ...
                    if dur <= 0:
                        ...
                        if "active" in h:
                            h.active = false
            elif "kind" in h and h.kind in ["bumper", "pinball_bumper", "stationary_bumper"]:
