...
func execute(strategy: String, delta: float):
	...
	if typeof(self.ball) == TYPE_DICTIONARY:
		s_timer = float(self.ball.get("slime_trail_booster_timer", 0.0))
	else:
		if "slime_trail_booster_timer" in self.ball:
			s_timer = float(self.ball.slime_trail_booster_timer)
		elif self.ball.has_method("get_meta") and self.ball.has_meta("slime_trail_booster_timer"):
			s_timer = float(self.ball.get_meta("slime_trail_booster_timer"))
	...
	if s_timer > 0:
		...
		if typeof(self.ball) == TYPE_DICTIONARY:
			self.ball["slime_trail_booster_timer"] = s_timer
		else:
			if "slime_trail_booster_timer" in self.ball:
				self.ball.slime_trail_booster_timer = s_timer
			if self.ball.has_method("set_meta"):
				self.ball.set_meta("slime_trail_booster_timer", s_timer)
		...
		if self.world != null and typeof(self.world) == TYPE_OBJECT and "arena" in self.world and self.world.arena != null:
			if "hazards" in self.world.arena:
				...
				if typeof(self.ball) == TYPE_DICTIONARY:
					ball_x = float(self.ball.get("x", 0.0))
					ball_y = float(self.ball.get("y", 0.0))
					last_x = float(self.ball.get("slime_last_x", ball_x))
					last_y = float(self.ball.get("slime_last_y", ball_y))
					ball_team = str(self.ball.get("team", self.ball.get("ball_type", "")))
					ball_id = self.ball.get("id", null)
				else:
					ball_x = float(self.ball.x) if "x" in self.ball else 0.0
					ball_y = float(self.ball.y) if "y" in self.ball else 0.0
					if self.ball.has_method("get_meta") and self.ball.has_meta("slime_last_x"):
						last_x = float(self.ball.get_meta("slime_last_x"))
					else:
						last_x = float(self.ball.slime_last_x) if "slime_last_x" in self.ball else ball_x
					if self.ball.has_method("get_meta") and self.ball.has_meta("slime_last_y"):
						last_y = float(self.ball.get_meta("slime_last_y"))
					else:
						last_y = float(self.ball.slime_last_y) if "slime_last_y" in self.ball else ball_y
					ball_team = str(self.ball.team) if "team" in self.ball else (str(self.ball.ball_type) if "ball_type" in self.ball else "")
					ball_id = self.ball.id if "id" in self.ball else null

				var dist_moved_sq = (ball_x - last_x) * (ball_x - last_x) + (ball_y - last_y) * (ball_y - last_y)
				if dist_moved_sq > 400.0:
					if typeof(self.ball) == TYPE_DICTIONARY:
						self.ball["slime_last_x"] = ball_x
						self.ball["slime_last_y"] = ball_y
					else:
						if "slime_last_x" in self.ball:
							self.ball.slime_last_x = ball_x
						elif self.ball.has_method("set_meta"):
							self.ball.set_meta("slime_last_x", ball_x)
						if "slime_last_y" in self.ball:
							self.ball.slime_last_y = ball_y
						elif self.ball.has_method("set_meta"):
							self.ball.set_meta("slime_last_y", ball_y)

					var rng = RandomNumberGenerator.new()
					rng.randomize()
					var h_id = 900000 + (rng.randi() % 100000)
					var hazard = {
						"id": h_id,
						"x": ball_x,
						"y": ball_y,
	...
	if boosters.size() > 0:
		...
		if dist <= ball_radius + 10:
			...
			if is_disabled_sf:
				...
				elif nearest_kind == "slime_trail_booster":
					if typeof(self.ball) == TYPE_DICTIONARY:
						...
						if "slime_trail_booster_timer" in self.ball:
							...
							self.ball.set_meta("slime_trail_booster_timer", 10.0)
				if self.world != null and "arena" in self.world and "hazards" in self.world.arena:
					var idx = self.world.arena.hazards.find(nearest)
					if idx != -1:
						self.world.arena.hazards.remove_at(idx)
