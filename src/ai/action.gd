...
func _collect_booster(delta: float):
    ...
    if boosters.size() > 0:
        ...
        if dist <= ball_radius + 10:
            ...
            elif "kind" in nearest and nearest.kind == "night_vision_booster":
                var traits = []
                if "traits" in self.ball:
                    traits = self.ball.traits
                elif self.ball.has_method("get_meta") and self.ball.has_meta("traits"):
                    traits = self.ball.get_meta("traits")
                if not traits.has("night_vision"):
                    traits.append("night_vision")
                    if self.ball.has_method("set_meta"):
                        self.ball.set_meta("traits", traits)
                    else:
                        self.ball.traits = traits

                var base_p = 250.0
                if "base_perception_radius" in self.ball:
                    base_p = self.ball.base_perception_radius
                elif self.ball.has_method("get_meta") and self.ball.has_meta("base_perception_radius"):
                    base_p = self.ball.get_meta("base_perception_radius")
                elif "perception_radius" in self.ball:
                    base_p = self.ball.perception_radius

                base_p += 500.0
                if self.ball.has_method("set_meta"):
                    self.ball.set_meta("base_perception_radius", base_p)
                else:
    ...
    if pull_timer > 0:
        ...
        if self.world != null and "arena" in self.world and "hazards" in self.world.arena:
            for hazard in self.world.arena.hazards:
                ...
                var pullable = ["healing_spring", "booster", "drone_item", "stealth_drone_item", "shadow_booster", "decoy_item", "silence_booster", "placeable_trap_item", "exit_portal_item", "position_swap_item", "magnet_booster", "stamina_booster", "night_vision_booster", "link_booster", "weather_booster", "portal_gun_item"]
