...
func _spawn_skill_particles(skill_name: String = ""):
	...
	_check_synergy(skill_name, self.ball.x, self.ball.y, 150.0)


func _check_synergy(base_skill: String, x: float, y: float, radius: float):
	var allies = _get_allies()
	...
	for ally in allies:
		...
		if "alive" in ally:
			is_alive = ally.alive
		elif ally.has_method("has_meta") and ally.has_meta("alive"):
			is_alive = ally.get_meta("alive")
		if not is_alive or ("id" in ally and ally.id == self.ball.id):
			continue
		...
		if dist_sq <= radius * radius:
			var ally_skill = ""
			if "skill" in ally:
				ally_skill = ally.skill
			elif "SKILL" in ally:
				ally_skill = ally.SKILL

			var combo = [base_skill, ally_skill]
			combo.sort()
			var combo_str = combo[0] + "_" + combo[1]

			if combo_str == "elemental_burst_wave_attack":
				_apply_synergy_steam_explosion(x, y)
				synergy_triggered = true
				break
			elif combo_str == "dash_rage_burst":
				_apply_synergy_meteor_strike(x, y)
				synergy_triggered = true
				break
			elif combo_str == "explosion_fireball":
				_apply_synergy_hellfire(x, y)
				synergy_triggered = true
				break
			elif combo_str == "heal_ally_protect_ally" or combo_str == "heal_ally_holy_shield":
				_apply_synergy_sanctuary(x, y)
				synergy_triggered = true
				break

	if synergy_triggered:
		if self.ball.has_method("set_meta"):
			self.ball.set_meta("team_message", {"type": "synergy_triggered", "radius": radius * 1.5})
		elif "team_message" in self.ball:
			self.ball.team_message = {"type": "synergy_triggered", "radius": radius * 1.5}

func _apply_synergy_steam_explosion(x: float, y: float):
	var enemies = _get_enemies()
	var radius = 200.0
	for e in enemies:
		var dx = e.x - x
...
func _apply_synergy_meteor_strike(x: float, y: float):
	var enemies = _get_enemies()
	var radius = 100.0
	for e in enemies:
		var dx = e.x - x
		...
		if dx*dx + dy*dy <= radius*radius:
			if e.has_method("take_damage"):
				e.take_damage(80.0)
			if "stutter_timer" in e:
				e.stutter_timer += 1.0
			elif e.has_method("set_meta"):
				var curr = 0.0
				if e.has_meta("stutter_timer"): curr = e.get_meta("stutter_timer")
				e.set_meta("stutter_timer", curr + 1.0)

func _apply_synergy_hellfire(x: float, y: float):
	var enemies = _get_enemies()
	var radius = 150.0
	for e in enemies:
		var dx = e.x - x
		var dy = e.y - y
		if dx*dx + dy*dy <= radius*radius:
			if "dot_duration" in e:
				e.dot_duration = max(float(e.dot_duration), 5.0)
				e.dot_damage_per_tick = 10.0
			elif e.has_method("set_meta"):
				var curr = 0.0
				if e.has_meta("dot_duration"): curr = e.get_meta("dot_duration")
				e.set_meta("dot_duration", max(curr, 5.0))
				e.set_meta("dot_damage_per_tick", 10.0)

func _apply_synergy_sanctuary(x: float, y: float):
	var allies = _get_allies()
	var radius = 150.0
	for a in allies:
		var dx = a.x - x
