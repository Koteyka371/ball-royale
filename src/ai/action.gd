...
func execute(strategy: String, delta: float):
	...
	if typeof(self.world) == TYPE_OBJECT and "arena" in self.world and self.world.arena != null and "hazards" in self.world.arena:
		for hazard in self.world.arena.hazards:
			var haz_imm_active = false
			if "hazard_immunity_timer" in self.ball and float(self.ball.hazard_immunity_timer) > 0:
				haz_imm_active = true
			...
			elif hazard.has_method("get_meta") and hazard.has_meta("kind"): h_kind_for_skip = hazard.get_meta("kind")

			var has_damage = false
			if "damage" in hazard and float(hazard.damage) > 0:
				has_damage = true
	...
	if typeof(self.ball) == TYPE_DICTIONARY:
		...
		if self.ball.has_method("has_meta") and self.ball.has_meta("time_rewind_booster_active") and self.ball.get_meta("time_rewind_booster_active"):
			is_rewind = true
		elif "time_rewind_booster_active" in self.ball and self.ball.time_rewind_booster_active:
			is_rewind = true
	...
	if is_rewind:
		...
		if hp <= 0.0:
			if typeof(self.ball) == TYPE_DICTIONARY:
				...
				self.ball.set_meta("time_rewind_booster_active", false)
			...
			if history.size() > 0:
				...
				if typeof(self.ball) == TYPE_DICTIONARY:
					...
					if "hp" in self.ball: self.ball.hp = past.get("hp", self.ball.get("max_hp", 100.0))
					if "attack_timer" in self.ball and "attack_timer" in past: self.ball.attack_timer = past["attack_timer"]
					if "skill_timer" in self.ball and "skill_timer" in past: self.ball.skill_timer = past["skill_timer"]

				if typeof(self.world) == TYPE_DICTIONARY and self.world.has("events"):
					self.world.events.append({"type": "time_rewind", "data": {"id": self.ball.get("id", -1)}})
	...
	if self.ball.has_method("get_meta") and self.ball.has_meta("shuffle_booster_timer") and self.ball.get_meta("shuffle_booster_timer") > 0.0:
		...
		t -= delta
		self.ball.set_meta("shuffle_booster_timer", t)
		if t <= 0.0:
			var target = null
			if self.ball.has_meta("shuffle_booster_target"):
				target = self.ball.get_meta("shuffle_booster_target")

			var target_alive = false
			if typeof(target) == TYPE_OBJECT and target.has_method("get") and target.get("alive") != null:
			...
			if target != null and target_alive:
				var my_inv = []
				if self.ball.has_meta("inventory"): my_inv = self.ball.get_meta("inventory")
				var my_skill = null
				if self.ball.has_meta("active_skill"): my_skill = self.ball.get_meta("active_skill")
			...
			self.ball.set_meta("shuffle_booster_target", null)
	elif typeof(self.ball) == TYPE_DICTIONARY and self.ball.has("shuffle_booster_timer") and self.ball.shuffle_booster_timer > 0.0:
		self.ball.shuffle_booster_timer -= delta
		if self.ball.shuffle_booster_timer <= 0.0:
			var target = null
			if self.ball.has("shuffle_booster_target"):
				target = self.ball.shuffle_booster_target
			...
			if target != null and target_alive:
				var my_inv = []
				if self.ball.has("inventory"): my_inv = self.ball.inventory
				var my_skill = null
				if self.ball.has("active_skill"): my_skill = self.ball.active_skill
	...
	if "cursed_bumper_active" in self.ball: is_cursed_active = self.ball.cursed_bumper_active
	elif typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("get_meta") and self.ball.has_meta("cursed_bumper_active"): is_cursed_active = self.ball.get_meta("cursed_bumper_active")

	if is_cursed_active:
		if "stamina" in self.ball:
			self.ball.stamina = max(0.0, self.ball.stamina - 15.0 * delta)
			if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("stamina", self.ball.stamina)
		...
		if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("take_damage"):
			...
			if self.ball.hp <= 0:
				if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("alive", false)
				if "alive" in self.ball: self.ball.alive = false
			if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("hp", self.ball.hp)
	...
	if typeof(self.ball) == TYPE_DICTIONARY:
		...
		self.ball["state_history"].push_back({
		...
		if self.ball["state_history"].size() > 188:
		...
		if self.ball.has_method("has_meta") and self.ball.has_meta("state_history"):
			...
			history.push_back({
				...
				"y": float(self.ball.y),
				"hp": hp_val,
				"attack_timer": float(self.ball.get("attack_timer", 0.0) if "attack_timer" in self.ball else 0.0),
				"skill_timer": float(self.ball.get("skill_timer", 0.0) if "skill_timer" in self.ball else 0.0)
			})
			if history.size() > 188:
				history.pop_front()
	...
	if (strategy == "flee" or strategy == "defend" or strategy == "attack") and self.ball.has_meta("inventory"):
		...
		if inv.has("weather_shield"):
			...
			for stat in weather_statuses:
				var stat_val = 0.0
				if stat in self.ball: stat_val = self.ball.get(stat)
				elif self.ball is Dictionary and self.ball.has(stat): stat_val = self.ball.get(stat, 0.0)
				elif typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("has_meta") and self.ball.has_meta(stat): stat_val = self.ball.get_meta(stat)

				if stat_val > 0.0:
					if stat in self.ball: self.ball.set(stat, 0.0)
					elif self.ball is Dictionary: self.ball[stat] = 0.0
						...
						if dist < b_radius:
							...
							if not already_hit:
								...
								if typeof(hazard) == TYPE_DICTIONARY:
									...
									hazard_damage = hazard.damage if "damage" in hazard else 50.0
								...
								if typeof(self.ball) == TYPE_DICTIONARY:
									...
									self.ball.speed_debuff_timer = max(self.ball.speed_debuff_timer if "speed_debuff_timer" in self.ball else 0.0, 5.0)
									self.ball.speed_debuff_multiplier = min(self.ball.speed_debuff_multiplier if "speed_debuff_multiplier" in self.ball else 1.0, 0.5)
				...
				if is_disabled_sf:
					...
					if not hazard.has_meta("last_updated_tick") or hazard.get_meta("last_updated_tick") != current_tick:
						...
						if is_ts:
							...
							if "arena" in self.world and self.world.arena != null and "hazards" in self.world.arena:
								for other_hazard in self.world.arena.hazards:
									if other_hazard.kind in ["chain_lightning_active", "lightning_strike"]:
										var dx = other_hazard.x - hazard.x
										var dy = other_hazard.y - hazard.y
										var dist_sq = dx*dx + dy*dy
										if dist_sq < 22500:
								...
								if "balls" in self.world:
									for b in self.world.balls:
										...
										if b_alive:
											...
											if d_sq > 0.01:
												entities.append({"d_sq": d_sq, "b": b})

								entities.sort_custom(Callable(self, "_sort_by_d_sq_desc"))

								var num_targets = min(3, entities.size())
								for i in range(num_targets):
									var t = entities[i].b
									var m_id = "cb_" + str(randi() % 1000000)
									var HazardClass = load("res://arena/procedural_arena.gd").Hazard if ResourceLoader.exists("res://arena/procedural_arena.gd") else null
									var m = null
									if HazardClass:
										m = HazardClass.new(m_id, hazard.x, hazard.y, 10.0, "conductor_bolt", 40.0)
									else:
										m = {"id": m_id, "x": hazard.x, "y": hazard.y, "radius": 10.0, "kind": "conductor_bolt", "damage": 40.0}

									var t_id = null
									if "id" in t: t_id = t.id

									if typeof(m) == TYPE_OBJECT:
										m.set_meta("target_id", t_id)
										m.set_meta("duration", 5.0)
									else:
										m["target_id"] = t_id
										m["duration"] = 5.0
					...
					if "tick" in self.world:
						current_tick = self.world.tick
					if not (typeof(hazard) == TYPE_OBJECT and hazard.has_method("has_meta") and hazard.has_meta("last_updated_tick")) and not (typeof(hazard) == TYPE_DICTIONARY and hazard.has("last_updated_tick")) or (typeof(hazard) == TYPE_OBJECT and hazard.get_meta("last_updated_tick") != current_tick) or (typeof(hazard) == TYPE_DICTIONARY and hazard.get("last_updated_tick") != current_tick):
						if typeof(hazard) == TYPE_OBJECT: hazard.set_meta("last_updated_tick", current_tick)
						elif typeof(hazard) == TYPE_DICTIONARY: hazard["last_updated_tick"] = current_tick

						var target_id = null
						if typeof(hazard) == TYPE_OBJECT and hazard.has_meta("target_id"): target_id = hazard.get_meta("target_id")
						elif typeof(hazard) == TYPE_DICTIONARY and hazard.has("target_id"): target_id = hazard["target_id"]
						...
						if target != null:
							...
							if dist > 0.01:
								...
								var move_dist = min(speed, dist)
								hazard.x += (dx/dist) * move_dist
								hazard.y += (dy/dist) * move_dist

							var h_radius = 10.0
							if "radius" in hazard: h_radius = hazard.radius
							var t_radius = 15.0
							if "radius" in target: t_radius = target.radius

							if dist < (h_radius + t_radius):
								var h_dmg = 40.0
								if "damage" in hazard: h_dmg = hazard.damage
								if typeof(target) == TYPE_OBJECT and target.has_method("take_damage"):
									target.take_damage(h_dmg)
								else:
									var t_hp = 100
									if "hp" in target: t_hp = target.hp
									target.hp = t_hp - h_dmg

								if typeof(hazard) == TYPE_OBJECT: hazard.set_meta("duration", 0.0)
				...
				if is_disabled_sf:
					continue
						...
						if ball_type != "sniper":
							...
							if trap_variant == "poison":
								...
								if world != null and world.has_method("get_arena") and world.get_arena() != null and "hazards" in world.get_arena():
									var gw = null
									if load("res://src/arena/procedural_arena.gd") != null:
										gw = load("res://src/arena/procedural_arena.gd").Hazard.new()
									if gw != null:
										gw.id = world.get_arena().hazards.size() + 8500
										gw.x = hazard.x
										gw.y = hazard.y
										gw.radius = 200.0
										gw.kind = "gravity_well"
										gw.damage = 0.0
								...
								if "damage" in hazard and hazard.damage > 0:
									current_damage = hazard.damage

								if "hp" in self.ball:
									self.ball.hp -= current_damage
									if self.ball.hp <= 0:
										self.ball.alive = false
										if world != null and world.has_method("add_event"):
											world.add_event("kill", {"killer_id": hazard.get_meta("owner_id") if hazard.has_method("get_meta") and hazard.has_meta("owner_id") else -1, "victim_id": self.ball.id})
						...
						if dist < hazard.radius * 0.5:
							...
							if hazard.kind in ["firenado", "local_firenado"]:
								if "burn_timer" in self.ball:
									self.ball.burn_timer = max(self.ball.burn_timer, 5.0)
								elif self.ball.has_method("set_meta"):
									var current = self.ball.get_meta("burn_timer") if self.ball.has_meta("burn_timer") else 0.0
									self.ball.set_meta("burn_timer", max(current, 5.0))
							elif hazard.kind in ["poison_tornado", "local_poison_tornado"]:
								if "poison_timer" in self.ball:
									self.ball.poison_timer = max(self.ball.poison_timer, 5.0)
								elif self.ball.has_method("set_meta"):
									var current = self.ball.get_meta("poison_timer") if self.ball.has_meta("poison_timer") else 0.0
									self.ball.set_meta("poison_timer", max(current, 5.0))
						...
						if d < (b_rad + hazard.radius):
							...
							if powerup == "cursed":
								var is_cursed = false
								if "cursed_bumper_active" in self.ball: is_cursed = self.ball.cursed_bumper_active
								elif typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("get_meta") and self.ball.has_meta("cursed_bumper_active"): is_cursed = self.ball.get_meta("cursed_bumper_active")

								if not is_cursed:
									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("cursed_bumper_active", true)
									if "cursed_bumper_active" in self.ball: self.ball.cursed_bumper_active = true
									...
									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta") and not self.ball.has_meta("base_damage_multiplier"): self.ball.set_meta("base_damage_multiplier", dmg_mult)
									if "damage_multiplier" in self.ball and not "base_damage_multiplier" in self.ball: self.ball.base_damage_multiplier = dmg_mult

									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("damage_multiplier", dmg_mult * 3.0)
									if "damage_multiplier" in self.ball: self.ball.damage_multiplier = dmg_mult * 3.0

									var spd = 2.0
									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("has_meta") and self.ball.has_meta("speed"): spd = self.ball.get_meta("speed")
									elif "speed" in self.ball: spd = self.ball.speed

									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta") and not self.ball.has_meta("base_speed"): self.ball.set_meta("base_speed", spd)
									if "speed" in self.ball and not "base_speed" in self.ball: self.ball.base_speed = spd

									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("speed", spd * 3.0)
									if "speed" in self.ball: self.ball.speed = spd * 3.0
								...
								if is_cursed:
									...
									if "cursed_bumper_active" in self.ball: self.ball.cursed_bumper_active = false
									...
									if "damage_multiplier" in self.ball: self.ball.damage_multiplier = base_dmg_mult

									var base_spd = 2.0
									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("has_meta") and self.ball.has_meta("base_speed"): base_spd = self.ball.get_meta("base_speed")
									elif "base_speed" in self.ball: base_spd = self.ball.base_speed

									if typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("set_meta"): self.ball.set_meta("speed", base_spd)
	...
	elif typeof(self.ball) == TYPE_OBJECT and self.ball.has_method("has_meta") and self.ball.has_meta("shield_booster_active"):
		shield_active = self.ball.get_meta("shield_booster_active")

	if damage_taken > 0 and shield_active:
		if typeof(self.ball) == TYPE_DICTIONARY:
			self.ball["hp"] = start_hp
			...
			self.ball.hp = start_hp
			if self.ball.has_method("set_meta"):
				self.ball.set_meta("shield_booster_active", false)
		damage_taken = 0
	...
	if delta > 0:
		...
		var haz_imm_timer = 0.0
		if "hazard_immunity_timer" in self.ball:
			haz_imm_timer = float(self.ball.hazard_immunity_timer)
		elif self.ball.has_method("get_meta") and self.ball.has_meta("hazard_immunity_timer"):
			haz_imm_timer = float(self.ball.get_meta("hazard_immunity_timer"))
		if haz_imm_timer > 0:
			haz_imm_timer -= delta
			if haz_imm_timer < 0:
				haz_imm_timer = 0.0
			if "hazard_immunity_timer" in self.ball:
				self.ball.hazard_immunity_timer = haz_imm_timer
			if self.ball.has_method("set_meta"):
		...
		if typeof(entities) == TYPE_DICTIONARY and entities.has("enemies"):
			for e in entities["enemies"]:
				...
				if e_type != "spectator" and not e_flying:
					...
					elif e.has_method("has_meta") and e.has_meta("silencer_timer"): sil = e.get_meta("silencer_timer")
					if sil > 0.0: target_pr *= 0.5
					var dx = (e.x if "x" in e else e.get_meta("x")) - self.ball.x
					var dy = (e.y if "y" in e else e.get_meta("y")) - self.ball.y
		...
		for b in self.world.balls:
			...
			if alive and not is_decoy and not is_illusion:
				...
				if e_type != my_type and e_type != "spectator" and not e_flying:
					...
					var target_pr = perception_radius
					var sil = 0.0
					if "silencer_timer" in b: sil = b.silencer_timer
					elif b.has_method("has_meta") and b.has_meta("silencer_timer"): sil = b.get_meta("silencer_timer")
					if sil > 0.0: target_pr *= 0.5
...
func _use_skill():
	...
	if skill_timer <= 0.0 and self.ball.has_method("use_skill"):
		...
		if skill_name == "energy_shield":
			...
			elif self.world.has_method("get_balls"):
			...
			for e in all_entities:
				...
				var e_type = ""
				if "ball_type" in e: e_type = e.ball_type
				...
				for i in range(3):
					...
					if decoy != null:
						...
						if typeof(decoy) == TYPE_DICTIONARY:
							...
							var mhp = 100.0
							if typeof(self.ball) == TYPE_DICTIONARY and self.ball.has("max_hp"): mhp = self.ball["max_hp"]
							elif "max_hp" in self.ball: mhp = self.ball.max_hp

							decoy["max_hp"] = mhp * 0.5
							decoy["hp"] = decoy["max_hp"]

							var dam = 10.0
							if typeof(self.ball) == TYPE_DICTIONARY and self.ball.has("damage"): dam = self.ball["damage"]
							elif "damage" in self.ball: dam = self.ball.damage
							decoy["damage"] = dam * 0.5
							...
							if decoy.has("x") and decoy.has("y"):
								decoy["x"] += cos(angle) * 20.0
								decoy["y"] += sin(angle) * 20.0
							...
							if "damage" in decoy:
								var dam = 10.0
								if "damage" in self.ball: dam = self.ball.damage
								decoy.damage = dam * 0.5
			...
			if enemies.size() > 0:
				...
				if target != null:
					...
					if dist > 0:
						end_x = self.ball.x + (dx/dist) * 300.0
						end_y = self.ball.y + (dy/dist) * 300.0
			...
			var hazard_line = {
				"id": "hazard_line_" + str(b_id) + "_" + str(self.world.tick),
				"kind": "deployable_thin_hazard_line",
				"x": self.ball.x,
				"y": self.ball.y,
				"start_x": self.ball.x,
			...
			if "arena" in self.world and self.world.arena != null:
				if "hazards" in self.world.arena:
					self.world.arena.hazards.append(hazard_line)
...
func _apply_friendly_aura(delta: float):
	...
	if not is_dashing and stutter <= 0.0:
		...
		if self.ball.has_method("has_meta") and self.ball.has_meta("is_exhausted"):
			is_exh = self.ball.get_meta("is_exhausted")
		elif "is_exhausted" in self.ball:
			is_exh = self.ball.is_exhausted
		...
		var sc_timer_aura = 0.0
		if self.ball.has_method("has_meta") and self.ball.has_meta("supercharge_timer"):
			sc_timer_aura = float(self.ball.get_meta("supercharge_timer"))
		elif "supercharge_timer" in self.ball:
			sc_timer_aura = float(self.ball.supercharge_timer)

		if sc_timer_aura > 0.0:
			if "speed" in self.ball:
				self.ball.speed *= 1.5
			if "damage" in self.ball:
				self.ball.damage *= 1.5
...
func _update_skill_timer(delta: float):
	...
	if typeof(self.ball) != TYPE_DICTIONARY and self.ball.has_method("has_meta") and self.ball.has_meta("supercharge_timer"):
		...
		sc_timer = float(self.ball.supercharge_timer)

	if sc_timer > 0.0:
		sc_timer -= delta
		var drain_amount = 5.0 * delta
	...
	if "gravity_well_aura_timer" in self.ball:
		gravity_well_aura_timer = float(self.ball.gravity_well_aura_timer)
	elif self.ball.has_method("get_meta") and self.ball.has_meta("gravity_well_aura_timer"):
		gravity_well_aura_timer = self.ball.get_meta("gravity_well_aura_timer")
	if gravity_well_aura_timer > 0:
		gravity_well_aura_timer -= delta
		if "gravity_well_aura_timer" in self.ball:
			self.ball.gravity_well_aura_timer = gravity_well_aura_timer
		elif self.ball.has_method("set_meta"):
		...
		if self.world != null and "balls" in self.world:
			for b in self.world.balls:
				...
				if is_alive and b_type != "spectator" and b_id != self.ball.id and b_team != self.ball.team:
					var bx = 0.0
					var by = 0.0
					if typeof(b) == TYPE_DICTIONARY:
						if b.has("x"): bx = b.x
		...
		if self.world != null and "arena" in self.world and "hazards" in self.world.arena:
			for hazard in self.world.arena.hazards:
				...
				if h_kind == "booster_trap":
					...
					elif typeof(hazard) == TYPE_OBJECT and hazard.has_meta("owner_id"): h_owner_id = hazard.get_meta("owner_id")

					var b_id = null
					if "id" in self.ball: b_id = self.ball.id
					elif self.ball.has_meta("id"): b_id = self.ball.get_meta("id")

					if h_owner_id != null and h_owner_id != b_id:
						var h_x = 0.0
						if "x" in hazard: h_x = hazard.x
						elif hazard.has_method("get_meta") and hazard.has_meta("x"): h_x = hazard.get_meta("x")
						var h_y = 0.0
						...
						if dist_sq < h_rad * h_rad:
							var effects = ["poison", "freeze", "stun"]
							var effect = effects[randi() % effects.size()]
							if effect == "poison":
								var current_poison = 0.0
								if "poison_timer" in self.ball: current_poison = self.ball.poison_timer
								elif self.ball.has_method("get_meta") and self.ball.has_meta("poison_timer"): current_poison = self.ball.get_meta("poison_timer")
								var new_poison = 3.0
								if current_poison > new_poison: new_poison = current_poison
								if "poison_timer" in self.ball: self.ball.poison_timer = new_poison
								elif self.ball.has_method("set_meta"): self.ball.set_meta("poison_timer", new_poison)
							elif effect == "freeze":
								var current_freeze = 0.0
								if "freeze_timer" in self.ball: current_freeze = self.ball.freeze_timer
								elif self.ball.has_method("get_meta") and self.ball.has_meta("freeze_timer"): current_freeze = self.ball.get_meta("freeze_timer")
								var new_freeze = 3.0
								if current_freeze > new_freeze: new_freeze = current_freeze
								if "freeze_timer" in self.ball: self.ball.freeze_timer = new_freeze
								elif self.ball.has_method("set_meta"): self.ball.set_meta("freeze_timer", new_freeze)
							elif effect == "stun":
								var current_stun = 0.0
				...
				if h_kind == "disguised_trap":
					...
					var dist_sq = (h_x - self.ball.x)*(h_x - self.ball.x) + (h_y - self.ball.y)*(h_y - self.ball.y)
					var trigger_radius = 35.0
					if "radius" in hazard: trigger_radius = hazard.radius + 15.0
					elif hazard.has_method("get_meta") and hazard.has_meta("radius"): trigger_radius = hazard.get_meta("radius") + 15.0
					...
					if dist_sq < trigger_radius * trigger_radius:
						...
						if "speed" in self.ball and "base_speed" in self.ball:
							self.ball.speed = self.ball.base_speed * 0.1
	...
	elif self.ball.has_method("has_meta") and self.ball.has_meta("extended_mag_timer"): em_timer = self.ball.get_meta("extended_mag_timer")
	if em_timer > 0.0:
		cooldown_mult *= 2.0
		em_timer -= delta
	...
	elif self.ball.has_method("has_meta") and self.ball.has_meta("silencer_timer"): sil_timer = self.ball.get_meta("silencer_timer")
	if sil_timer > 0.0:
		sil_timer -= delta
	...
	elif self.ball.has_method("has_meta") and self.ball.has_meta("modified_scope_timer"): ms_timer = self.ball.get_meta("modified_scope_timer")
	if ms_timer > 0.0:
		ms_timer -= delta
		if self.ball.has_method("set_meta"): self.ball.set_meta("modified_scope_timer", ms_timer)
		else: self.ball.modified_scope_timer = ms_timer
	...
	if "stealth_booster_timer" in self.ball:
		stealth_booster_timer = float(self.ball.stealth_booster_timer)
	elif self.ball.has_method("get_meta") and self.ball.has_meta("stealth_booster_timer"):
		stealth_booster_timer = self.ball.get_meta("stealth_booster_timer")

	if stealth_booster_timer > 0:
		stealth_booster_timer -= delta
		if stealth_booster_timer < 0:
			stealth_booster_timer = 0.0
