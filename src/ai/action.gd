...
func _attempt_damage(attacker, target) -> void:
	...
	var target_is_necro = false
	var necro_target_b_type = ""
	if "ball_type" in target: necro_target_b_type = str(target.ball_type).to_lower()
	elif target.has_method("get_meta") and target.has_meta("ball_type"): necro_target_b_type = str(target.get_meta("ball_type")).to_lower()

	if necro_target_b_type == "necromancer":
		target_is_necro = true

	if target_is_necro:
		var stacks = 0
		if "bone_armor_stacks" in target: stacks = target.bone_armor_stacks
		elif target.has_method("get_meta") and target.has_meta("bone_armor_stacks"): stacks = target.get_meta("bone_armor_stacks")
		if stacks > 0:
			if "bone_armor_stacks" in target: target.bone_armor_stacks = stacks - 1
			elif target.has_method("set_meta"): target.set_meta("bone_armor_stacks", stacks - 1)
...
func execute(strategy: String, delta: float):
	...
	var b_type_exec = ""
	if "ball_type" in self.ball: b_type_exec = str(self.ball.ball_type).to_lower()
	elif self.ball.has_method("get_meta") and self.ball.has_meta("ball_type"): b_type_exec = str(self.ball.get_meta("ball_type")).to_lower()
	elif "BALL_TYPE" in self.ball: b_type_exec = str(self.ball.BALL_TYPE).to_lower()
	elif self.ball is Dictionary and self.ball.has("ball_type"): b_type_exec = str(self.ball["ball_type"]).to_lower()

	if b_type_exec == "necromancer":
		var ba_timer = 5.0
		if "bone_armor_timer" in self.ball: ba_timer = float(self.ball.bone_armor_timer)
		elif self.ball.has_method("get_meta") and self.ball.has_meta("bone_armor_timer"): ba_timer = float(self.ball.get_meta("bone_armor_timer"))

		var ba_stacks = 0
		if "bone_armor_stacks" in self.ball: ba_stacks = int(self.ball.bone_armor_stacks)
		elif self.ball.has_method("get_meta") and self.ball.has_meta("bone_armor_stacks"): ba_stacks = int(self.ball.get_meta("bone_armor_stacks"))

		ba_timer -= delta
		if ba_timer <= 0.0:
			ba_stacks = min(3, ba_stacks + 1)
			ba_timer = 5.0

		if "bone_armor_timer" in self.ball: self.ball.bone_armor_timer = ba_timer
		elif self.ball.has_method("set_meta"): self.ball.set_meta("bone_armor_timer", ba_timer)
		elif self.ball is Dictionary: self.ball["bone_armor_timer"] = ba_timer

		if "bone_armor_stacks" in self.ball: self.ball.bone_armor_stacks = ba_stacks
		elif self.ball.has_method("set_meta"): self.ball.set_meta("bone_armor_stacks", ba_stacks)
		elif self.ball is Dictionary: self.ball["bone_armor_stacks"] = ba_stacks
