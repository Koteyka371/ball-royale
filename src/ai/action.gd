			...
			continue
                if hazard.kind == "explosive_barrel":
                    ...
                    if not hazard.has_meta("last_updated_tick") or hazard.get_meta("last_updated_tick") != current_tick:
                        ...
                        if hazard.kind == "one_way_teleporter" and randf() < 0.01:
                            if self.world.arena.has_method("get_random_spawn_point"):
                                var pt = self.world.arena.get_random_spawn_point(25.0)
                                hazard.set_meta("target_x", pt[0])
                                hazard.set_meta("target_y", pt[1])
