    ...
    if boosters.size() > 0:
        ...
        if dist <= ball_radius + 10:
            if "kind" in nearest and nearest.kind == "grapple_booster":
                ...
                if self.ball.has_method("set_meta"):
                    self.ball.set_meta("stealth_booster_timer", 15.0)
...
func _update_skill_timer(delta: float):
    ...
    var stealth_b_timer = 0.0
    if "stealth_booster_timer" in self.ball:
        stealth_b_timer = float(self.ball.stealth_booster_timer)
    elif self.ball.has_method("get_meta") and self.ball.has_meta("stealth_booster_timer"):
        stealth_b_timer = self.ball.get_meta("stealth_booster_timer")

    if stealth_b_timer > 0.0:
        stealth_b_timer -= delta
        if stealth_b_timer < 0.0:
            stealth_b_timer = 0.0
        if "stealth_booster_timer" in self.ball:
            self.ball.stealth_booster_timer = stealth_b_timer
        elif self.ball.has_method("set_meta"):
            self.ball.set_meta("stealth_booster_timer", stealth_b_timer)
