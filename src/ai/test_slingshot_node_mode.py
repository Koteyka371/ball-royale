...
def test_slingshot_node_spawn():
    ...
    assert kind == "slingshot_node"

def test_slingshot_node_hook():
    world = MockWorld()
    ...
    node = MockHazard(600, 500, "slingshot_node")
    world.arena.hazards.append(node)

    action = Action(ball, world)
    action._use_skill()

    # Slingshot user away from target (target is right, so user goes left)
    assert ball.vx < 0.0
    assert getattr(ball, "is_frictionless", False) is True
    ...
    assert world.items[0]["material_type"] == "Elastic Cord"
