...
func scan() -> Dictionary:
    ...
    if self.world != null and "arena" in self.world and "hazards" in self.world.arena:
        for h in self.world.arena.hazards:
            if "kind" in h and (h.kind == "smokescreen" or h.kind == "breakable_wall" or h.kind == "smoke_zone"):
    ...
    for e in entities.get("enemies", []):
        ...
        if not revealed_by_flare:
            ...
            var e_has_invisibility_booster = false
            if e.has_method("get_meta") and e.has_meta("invisibility_booster_timer"):
                e_has_invisibility_booster = e.get_meta("invisibility_booster_timer") > 0
            elif "invisibility_booster_timer" in e:
                e_has_invisibility_booster = float(e.invisibility_booster_timer) > 0
