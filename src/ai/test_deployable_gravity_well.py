...
def test_deployable_gravity_well_collect_and_deploy():
    brawler = MockBall(1, 0, 0, team="teamA")

    booster = MockEntity(3, 0, 0, kind="deployable_gravity_well")
    ...
    assert "deployable_gravity_well" in brawler.inventory
    ...
    assert "deployable_gravity_well" not in brawler.inventory

    assert len(arena.hazards) == 1
    bh = arena.hazards[0]
    assert bh.kind == "gravity_well"
    assert bh.radius == 250.0
    assert getattr(bh, "damage", 0.0) == 0.0
    assert getattr(bh, "duration", 0.0) == 3.0
