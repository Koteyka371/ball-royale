        ...
        if boosters:
            ...
            if dist <= ball_radius + 10:
                if getattr(nearest, "kind", None) == "overclock_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "mass_stealth_booster":
                    self.ball.stealth_booster_timer = 10.0
                    # Apply stealth to nearby allies
                    allies = self._get_allies()
                    if allies:
                        for ally in allies:
                            # Distance check
                            if (ally.x - self.ball.x)**2 + (ally.y - self.ball.y)**2 <= 200.0**2:
