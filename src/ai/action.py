...
class Action:
    ...
    def _attempt_damage(self, attacker, target) -> None:
        ...
        if a_team == t_team and a_team != -1 and getattr(attacker, "id", None) != getattr(target, "id", None):
            if getattr(target, "friendly_fire_reflect_timer", 0.0) > 0.0:
                if hasattr(attacker, "take_damage"):
                    attacker.take_damage(getattr(attacker, "damage", 10.0))
                elif hasattr(attacker, "hp"):
                    attacker.hp -= getattr(attacker, "damage", 10.0)
                return
        import random
        ...
        if getattr(attacker, "blood_magic_timer", 0.0) > 0.0 and random.random() < 0.2:
            target.stun_timer = getattr(target, "stun_timer", 0.0) + 0.5
            if hasattr(self, "_spawn_directed_particles"):
                self._spawn_directed_particles(attacker, target, "blood_magic_stun")
            ...
            if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                ...
                for h in self.world.arena.hazards:
                    if getattr(h, 'is_disabled_by_flare', False):
                        continue
                    if getattr(h, "kind", "") in ["orbital_debris", "bone_wall"]:
                        hx = h.x
                        hy = h.y
    ...
    def execute(self, strategy: str, delta: float) -> None:
        ...
        if hasattr(self.ball, "thermal_boots_timer"):
            if self.ball.thermal_boots_timer > 0.0:
                self.ball.thermal_boots_timer = max(0.0, self.ball.thermal_boots_timer - delta)
                if self.ball.thermal_boots_timer <= 0.0:
        ...
        if getattr(self.ball, "charging_shockwave_shield_active", False):
            if getattr(self.ball, "charging_shockwave_shield_timer", 0.0) < 5.0:
                self.ball.charging_shockwave_shield_timer = getattr(self.ball, "charging_shockwave_shield_timer", 0.0) + delta
                if self.ball.charging_shockwave_shield_timer > 5.0:
        ...
        if strategy in ("flee", "defend", "attack") and hasattr(self.ball, "inventory") and "invisible_status_trap" in self.ball.inventory:
            if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                from arena.procedural_arena import Hazard
                import random
                trap_id = len(self.world.arena.hazards) + random.randint(1000, 9999)
                trap = Hazard(trap_id, self.ball.x, self.ball.y, 25.0, "invisible_status_trap", 0.0)
                setattr(trap, 'duration', 20.0)
                setattr(trap, 'owner_id', getattr(self.ball, 'id', None))
                setattr(trap, 'invisible', True)
                self.world.arena.hazards.append(trap)
                self.ball.inventory.remove("invisible_status_trap")

        if strategy in ("flee", "defend", "attack") and hasattr(self.ball, "inventory") and "booster_trap" in self.ball.inventory:
            if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                from arena.procedural_arena import Hazard
                import random
                trap_id = len(self.world.arena.hazards) + random.randint(1000, 9999)
                trap = Hazard(trap_id, self.ball.x, self.ball.y, 15.0, "booster_trap", 0.0)
                setattr(trap, 'duration', 10.0)
                setattr(trap, 'owner_id', getattr(self.ball, 'id', None))
                self.world.arena.hazards.append(trap)
                self.ball.inventory.remove("booster_trap")

        if strategy in ("flee", "defend", "attack") and hasattr(self.ball, "inventory") and "zero_gravity_trap" in self.ball.inventory:
            if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                from arena.procedural_arena import Hazard
                import random
                trap_id = len(self.world.arena.hazards) + random.randint(1000, 9999)
                trap = Hazard(trap_id, self.ball.x, self.ball.y, 60.0, "zero_gravity_trap_hazard", 0.0)
                setattr(trap, 'duration', 10.0)
                setattr(trap, 'owner_id', getattr(self.ball, 'id', None))
                self.world.arena.hazards.append(trap)
                self.ball.inventory.remove("zero_gravity_trap")
        ...
        if strategy in ("flee", "defend", "attack") and hasattr(self.ball, "inventory") and "hookshot" in self.ball.inventory:
            ...
            in_danger = False
            # Check if in lava or out of bounds
            ...
            if self.ball.x < 0 or self.ball.x > arena_width or self.ball.y < 0 or self.ball.y > arena_height:
                in_danger = True
            ...
            if not in_danger and hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                for h in self.world.arena.hazards:
                    if getattr(h, "kind", "") == "lava":
                        dist_sq = (self.ball.x - h.x)**2 + (self.ball.y - h.y)**2
                        rad = getattr(h, "radius", 100)
                        if dist_sq < rad**2:
                            in_danger = True
                            break

            if in_danger or strategy == "flee":
                pull_dist = 400.0
                ...
                # Grapple to wall to save oneself
                ...
                if getattr(hazard, "kind", "") == "thrown_disruptor_bomb":
                    if getattr(hazard, "duration", 0.0) > 0:
                        ...
                        if hazard.duration <= 0:
                            ...
                            if hasattr(self.world, "events"):
                                self.world.events.append({'type': 'visual_effect', 'data': {'type': 'disruptor_explosion', 'x': hazard.x, 'y': hazard.y, 'radius': 150.0}})
                                ...
                                for b in self.world.balls:
                                    if getattr(b, "alive", True) and getattr(b, "team", None) != owner_team:
                                        ...
                                        if dx*dx + dy*dy <= 150.0**2:
                                            b.aura_disruption_timer = max(getattr(b, "aura_disruption_timer", 0.0), 10.0)
                ...
                if kind in ("gravity_well", "repulsor") and getattr(hazard, "active", True):
                    ...
                    if getattr(self.ball, "anchor_booster_timer", 0.0) <= 0:
                        ...
                        if dist_sq > 0.0001 and dist_sq < eff_radius * eff_radius:
                            ...
                            if hasattr(self.ball, "inventory") and "gravity_boots" in getattr(self.ball, "inventory", []):
                                force *= 0.1
...
# Handle minion decay
                    ...
                    if hazard.kind in ("explosive_barrel", "volatile_barrel"):
                                ...
                                if b_id not in hit_ids:
                                    ...
                                    if getattr(self.ball, "reflect_shield_active", False):
                                        ...
                                        if shield_cap <= 0:
                                            ...
                                            self.ball.reflect_shield_capacity = shield_cap
                                    else:
                                        mitigation = getattr(self.ball, "damage_mitigation", 0.0)
                                ...
                                if hazard.kind == "chaos_portal":
                                    ...
                                    for h in self.world.arena.hazards:
                                        ...
                                        if h.id == pair_id:
                                            ...
                                            break
                                    if paired_hazard:
                                        self.ball.x = paired_hazard.x
                                        self.ball.y = paired_hazard.y
                                        # Randomize velocity
                                        import random
                                        speed = math.hypot(getattr(self.ball, 'vx', 0.0), getattr(self.ball, 'vy', 0.0))
                                        if speed == 0:
                                            speed = getattr(self.ball, 'base_speed', 150.0)
                                        ...
                                        self.ball.confusion_timer = max(getattr(self.ball, 'confusion_timer', 0.0), 1.0)
                        ...
                        if dist_sq < hazard.radius * hazard.radius:
                            ...
                            if dist > 0.1:
                                ...
                                orbit_strength = 150.0 * delta
                                if getattr(self.ball, "anchor_booster_timer", 0.0) <= 0:
                                    self.ball.x += nx * pull_strength + px * orbit_strength
                                    self.ball.y += ny * pull_strength + py * orbit_strength
                        ...
                        if dist_sq < hazard.radius * hazard.radius:
                            ...
                            if dist_sq > 0.0001 and getattr(self.ball, "anchor_booster_timer", 0.0) <= 0:
                                ...
                                self.ball.x += nx * pull_strength
                                self.ball.y += ny * pull_strength
                    elif hazard.kind in ("black_hole", "clone_black_hole", "massive_black_hole", "mini_black_hole", "tornado", "firenado", "local_firenado", "poison_tornado", "local_poison_tornado", "portal", "teleporter", "one_way_teleporter", "swap_portal", "lightning_storm", "chaos_portal"):
                            ...
                            if ball_type != "sniper":
                                ...
                                if trap_variant == "poison":
                                    ...
                                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                                        ...
                                        if trap_variant == "mini_black_hole":
                                            bh = Hazard(bh_id, hazard.x, hazard.y, 75.0, "mini_black_hole", 0.0)
                                        else:
        ...
        if damage_taken > 0 and getattr(self.ball, "is_hologram", False):
            ...
            if hasattr(self.ball, "alive"):
                self.ball.alive = False
            return
        if damage_taken > 0 and getattr(self.ball, "charging_shockwave_shield_active", False) and getattr(self.ball, "charging_shockwave_shield_timer", 0.0) >= 5.0:
            ...
            if hasattr(self.world, "balls"):
                for other_ball in self.world.balls:
                    if other_ball != self.ball and getattr(other_ball, "alive", True) and getattr(other_ball, "team", "") != getattr(self.ball, "team", "unknown"):
                        dx = other_ball.x - self.ball.x
                        dy = other_ball.y - self.ball.y
                        dist = math.hypot(dx, dy)
                        if 0 < dist <= shockwave_radius:
                            other_ball.x += (dx / dist) * push_force * 0.1
                            other_ball.y += (dy / dist) * push_force * 0.1
                            other_ball.vx = getattr(other_ball, "vx", 0.0) + (dx / dist) * push_force
                            other_ball.vy = getattr(other_ball, "vy", 0.0) + (dy / dist) * push_force
                            if hasattr(other_ball, "stun_timer"):
    ...
    def _collect_booster(self, delta: float) -> None:
        ...
        if boosters:
            ...
            if dist <= ball_radius + 10:
                if getattr(nearest, "kind", None) == "anvil_piece":
                    if hasattr(self.world, "mode") and getattr(self.world.mode, "name", "") == "Blacksmith Boss":
                        ...
                        if not hasattr(self.world, "events"):
                            self.world.events = []
                        self.world.events.append({"type": "anvil_piece_collected", "count": self.world.mode.anvil_pieces_collected, "x": self.ball.x, "y": self.ball.y})
                ...
                elif getattr(nearest, "kind", None) == "legendary_loot":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("legendary_loot")
                    self.ball.damage = getattr(self.ball, "damage", 10.0) * 3.0
                    self.ball.speed = getattr(self.ball, "speed", 100.0) * 1.5

                    if not hasattr(self.world, "events"):
                        self.world.events = []
                    self.world.events.append({"type": "legendary_loot_collected", "x": self.ball.x, "y": self.ball.y})
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "friendly_fire_reflect_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "material_magnet_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "exploding_booster":
                ...
                elif getattr(nearest, "kind", None) == "insulator_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "decoy_flare_item":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("deployable_flare")
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        if nearest in self.world.arena.hazards:
                            self.world.arena.hazards.remove(nearest)
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                ...
                elif getattr(nearest, "kind", None) == "hookshot_booster":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "thermal_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "cooling_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "hazmat_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "heavy_anchor_booster":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "forecast_booster":
                    ...
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("weather_shield")
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        if nearest in self.world.arena.hazards:
                            self.world.arena.hazards.remove(nearest)
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                ...
                elif getattr(nearest, "kind", None) == "invisible_status_trap_item":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("invisible_status_trap")
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        if nearest in self.world.arena.hazards:
                            self.world.arena.hazards.remove(nearest)
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "booster_trap_item":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("booster_trap")
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        if nearest in self.world.arena.hazards:
                            self.world.arena.hazards.remove(nearest)
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "zero_gravity_trap_item":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("zero_gravity_trap")
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        if nearest in self.world.arena.hazards:
                            self.world.arena.hazards.remove(nearest)
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "nemesis_drone_booster":
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        try:
                            from arena.procedural_arena import Hazard
                            import random
        ...
        if skill_timer <= 0 or can_recast:
            ...
            if skill_name == "energy_shield":
                ...
                if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                    ...
                    thrown_bomb.id = 19000 + len(self.world.arena.hazards)
                    ...
                    thrown_bomb.radius = 20.0
                    thrown_bomb.kind = "thrown_disruptor_bomb"
                ...
                if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                    class HazardLineNode:
                        pass
                    node = HazardLineNode()
                    node.id = f"hazard_line_{self.ball.id}_{self.world.tick}"
                    node.kind = "deployable_thin_hazard_line"
                    node.x = self.ball.x
                    node.y = self.ball.y
                    node.radius = 10.0
                    node.damage = 50.0
                ...
                if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                    ...
                    for i in range(3):
                        m_id = f"om_{getattr(self.ball, 'id', 'x')}_{random.randint(0,99999)}"
                        mine = Hazard(id=m_id, x=self.ball.x, y=self.ball.y, radius=8.0, kind="player_orbital_mine", damage=20.0)
                        setattr(mine, 'owner_id', getattr(self.ball, "id", None))
                        setattr(mine, 'orbit_angle', (2 * math.pi / 3) * i)
                        setattr(mine, 'orbit_radius', 60.0)
                    ...
                    if getattr(self.ball, "_cl_collision_cd", 0.0) <= 0.0:
                        ...
                        for _ in range(3):
                            ...
                            for e in nearby: # 'nearby' already contains allies and enemies, we filter enemies
                                if e not in bounced_enemies and getattr(e, "team", None) != getattr(self.ball, "team", None) and getattr(e, "alive", True) and getattr(e, "insulator_timer", 0.0) <= 0:
                                    d = (e.x - current_pos.x)**2 + (e.y - current_pos.y)**2
    ...
    def _update_skill_timer(self, delta: float) -> None:
        if getattr(self.ball, "blood_magic_timer", 0.0) > 0.0:
            ...
            if current_st > prev_st:
                # Skill was just used, take 10 health
                if hasattr(self.ball, "take_damage"):
                    self.ball.take_damage(10.0)
                elif hasattr(self.ball, "hp"):
                    self.ball.hp -= 10.0

                if hasattr(self.world, 'events'):
                    self.world.events.append({'type': 'visual_effect', 'data': {'type': 'blood_magic_cast', 'x': self.ball.x, 'y': self.ball.y}})

                self.ball.skill_timer = 0.0 # No cooldown
            self.ball._prev_skill_timer = getattr(self.ball, "skill_timer", 0.0)
            self.ball.blood_magic_timer -= delta
        ...
        if hasattr(self.ball, "damage_reflection_timer") and self.ball.damage_reflection_timer > 0:
            ...
            if self.ball.damage_reflection_timer <= 0:
                self.ball.damage_reflection_active = False
        ...
        if getattr(self.ball, "insulator_timer", 0.0) > 0:
            self.ball.insulator_timer -= delta
            if self.ball.insulator_timer < 0:
                self.ball.insulator_timer = 0.0
        ...
        if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
            ...
            for hazard in self.world.arena.hazards:
                ...
                if getattr(hazard, "kind", "") == "invisible_status_trap":
                    if getattr(hazard, "owner_id", None) != getattr(self.ball, "id", None):
                        dist_sq = (hazard.x - self.ball.x)**2 + (hazard.y - self.ball.y)**2
                        if dist_sq < getattr(hazard, "radius", 25.0)**2:
                            import random
                            effect = random.choice(["poison", "freeze", "stun", "silence", "blindness", "stutter"])
                            if effect == "poison":
                                self.ball.poison_timer = max(getattr(self.ball, "poison_timer", 0.0), 4.0)
                            elif effect == "freeze":
                                self.ball.freeze_timer = max(getattr(self.ball, "freeze_timer", 0.0), 3.0)
                            elif effect == "stun":
                                self.ball.stun_timer = max(getattr(self.ball, "stun_timer", 0.0), 2.5)
                            elif effect == "silence":
                                self.ball.silence_timer = max(getattr(self.ball, "silence_timer", 0.0), 4.0)
                            elif effect == "blindness":
                                self.ball.blindness_timer = max(getattr(self.ball, "blindness_timer", 0.0), 4.0)
                            elif effect == "stutter":
                ...
                if getattr(hazard, "kind", "") == "booster_trap":
                    if getattr(hazard, "owner_id", None) != getattr(self.ball, "id", None):
                        dist_sq = (hazard.x - self.ball.x)**2 + (hazard.y - self.ball.y)**2
                        if dist_sq < getattr(hazard, "radius", 15.0)**2:
                            import random
                            effect = random.choice(["poison", "freeze", "stun"])
                            if effect == "poison":
                                self.ball.poison_timer = max(getattr(self.ball, "poison_timer", 0.0), 3.0)
                            elif effect == "freeze":
                                self.ball.freeze_timer = max(getattr(self.ball, "freeze_timer", 0.0), 3.0)
                            elif effect == "stun":
                                self.ball.stun_timer = max(getattr(self.ball, "stun_timer", 0.0), 3.0)
                    ...
                    if getattr(hazard, "owner_id", None) != getattr(self.ball, "id", None):
                        ...
                        if dist_sq < getattr(hazard, "radius", 60.0)**2:
                            ...
                            if dist_to_center > 0.0001:
                                ...
                                # Push towards the edge of the arena
                                push_force = 200.0
        ...
        if hasattr(self.ball, "friendly_fire_reflect_timer") and self.ball.friendly_fire_reflect_timer > 0.0:
            ...
            if self.ball.friendly_fire_reflect_timer < 0.0:
                self.ball.friendly_fire_reflect_timer = 0.0
        ...
        if hasattr(self.ball, "stealth_booster_timer") and self.ball.stealth_booster_timer > 0:
            ...
            if self.ball.stealth_booster_timer < 0:
                self.ball.stealth_booster_timer = 0.0
