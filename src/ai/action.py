...
class Action:
    ...
    def execute(self, strategy: str, delta: float) -> None:
        ...
        if strategy in ("flee", "defend", "attack") and hasattr(self.ball, "inventory") and "inverted_gravity_trap_booster" in self.ball.inventory:
            if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                from arena.procedural_arena import Hazard
                import random
                trap_id = len(self.world.arena.hazards) + random.randint(1000, 9999)
                trap = Hazard(trap_id, self.ball.x, self.ball.y, 40.0, "inverted_gravity_trap", 0.0)
                setattr(trap, 'duration', 10.0)
                setattr(trap, 'owner_id', getattr(self.ball, 'id', None))
                self.world.arena.hazards.append(trap)
                self.ball.inventory.remove("inverted_gravity_trap_booster")
...
# Handle minion decay
        ...
        if boosters:
            ...
            if dist <= ball_radius + 10:
                if getattr(nearest, "kind", None) == "drone_item":
                    ...
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
                        self.world.boosters.remove(nearest)
                elif getattr(nearest, "kind", None) == "inverted_gravity_trap_booster":
                    if not hasattr(self.ball, "inventory"):
                        self.ball.inventory = []
                    self.ball.inventory.append("inverted_gravity_trap_booster")
                    if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                        if nearest in self.world.arena.hazards:
                            self.world.arena.hazards.remove(nearest)
                    if hasattr(self.world, "boosters") and nearest in self.world.boosters:
        ...
        if skill_timer <= 0:
            ...
            if skill_name == "energy_shield":
                ...
                if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                    ...
                    for h in hazards:
                        if getattr(h, "kind", "") not in ["healing_spring", "booster", "drone_item", "stealth_drone_item", "shadow_booster", "stealth_booster", "decoy_item", "silence_booster", "placeable_trap_item", "aura_inverter_trap_item", "aura_inverter_trap_booster", "inverted_gravity_trap_booster", "exit_portal_item", "position_swap_item", "portal_gun_item", "freeze_booster", "hazard_immunity_booster", "reverse_gravity_booster", "anchor_booster", "disruptor_booster", "emp_booster", "cursed_booster", "black_hole_grenade_booster", "status_absorber_item", "grapple_booster", "time_rewind_booster", "time_stop_booster", "instant_rewind_booster", "charging_shockwave_shield_booster", "shield_booster", "homing_missile_booster", "rearm_token", "skill_reroll_booster", "dummy_item", "gravity_well_booster", "disguised_trap"]:
                            dx = h.x - self.ball.x
                            dy = h.y - self.ball.y
                            dist_sq = dx*dx + dy*dy
                            if dist_sq < min_dist_sq:
    ...
    def _update_skill_timer(self, delta: float) -> None:
        ...
        # If gravity well is inverted, also violently push loose boosters and hazards outwards
        if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
            for hazard in self.world.arena.hazards:
                if getattr(hazard, "kind", "") == "gravity_well" and getattr(hazard, "is_inverted", False):
                    import math
                    eff_radius = getattr(hazard, "radius", 250.0)
                    push_strength = 200.0 * delta * 5.0 # Violent push

                    # Push items in boosters
                    if hasattr(self.world, "boosters"):
                        for item in self.world.boosters:
                            i_x = getattr(item, "x", 0.0) if hasattr(item, "x") else item.get("x", 0.0)
                            i_y = getattr(item, "y", 0.0) if hasattr(item, "y") else item.get("y", 0.0)
                            ...
                            if dist_sq < eff_radius * eff_radius:
                                ...
                                if dist > 0.0001:
                                    ...
                                    ny = dy / dist
                                    if isinstance(item, dict):
                                        item["x"] = i_x + nx * push_strength
                                        item["y"] = i_y + ny * push_strength
                                    else:
                                        if hasattr(item, "x"): item.x += nx * push_strength
                                        if hasattr(item, "y"): item.y += ny * push_strength

                    # Push items in hazards
                    for item in self.world.arena.hazards:
                        if item == hazard:
                            continue
                        i_kind = getattr(item, "kind", "")
                        if i_kind.endswith("_booster") or i_kind in ["health_pack", "cleanser", "rearm_token"]:
                            i_x = getattr(item, "x", 0.0)
                            i_y = getattr(item, "y", 0.0)
                ...
                if getattr(hazard, "kind", "") == "mine_bounce":
                    ...
                    if getattr(hazard, "owner_id", None) != getattr(self.ball, "id", None):
                        ...
                        if dist_sq < (getattr(self.ball, "radius", 10.0) + detonation_radius)**2:
                            ...
                            hazard.duration = 0.0
                if getattr(hazard, "kind", "") == "inverted_gravity_trap":
                    if getattr(hazard, "owner_id", None) != getattr(self.ball, "id", None):
                        dist_sq = (hazard.x - self.ball.x)**2 + (hazard.y - self.ball.y)**2
                        if dist_sq < 10000: # 100 range to trigger the trap
                            ...
                            if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
                                try:
                                    from arena.procedural_arena import Hazard
                                    gw_id = len(self.world.arena.hazards) + random.randint(10000, 99999)
                                    gw = Hazard(gw_id, hazard.x, hazard.y, 250.0, "gravity_well", 0.0)
                                    setattr(gw, 'duration', 4.0)
                                    setattr(gw, 'is_inverted', True)
