                ...
                for hazard in self.world.arena.hazards:
                    ...
                    if hazard.kind in ("explosive_barrel", "volatile_barrel"):
                        ...
                        if getattr(hazard, "active", True):
                            ...
                            dist_sq = dx * dx + dy * dy
                            if dist_sq < hazard.radius * hazard.radius:
                                if getattr(self.ball, "cosmetic", "").lower().replace(" ", "_") not in ["snowshoes", "snow_boots", "snow_tires", "spiked_tires"]:
                                    if hasattr(self.ball, "vx") and hasattr(self.ball, "vy"):
                                        self.ball.x += self.ball.vx * delta
                                        self.ball.y += self.ball.vy * delta
                        ...
                        dist_sq = dx * dx + dy * dy
                        if dist_sq < hazard.radius * hazard.radius:
                            if getattr(self.ball, "ball_type", "") == "snow_yeti" or getattr(self.ball, "cosmetic", "").lower().replace(" ", "_") in ["snowshoes", "snow_boots", "snow_tires", "spiked_tires"]:
    ...
    def _update_skill_timer(self, delta: float) -> None:
        ...
        if hasattr(self.world, "arena") and hasattr(self.world.arena, "hazards"):
            for hazard in self.world.arena.hazards:
                ...
                if getattr(hazard, "kind", "") == "mirage_hazard":
                    dist_sq = (hazard.x - self.ball.x)**2 + (hazard.y - self.ball.y)**2
                    trigger_radius = getattr(hazard, "radius", 60.0)
                    if dist_sq < trigger_radius * trigger_radius:
                        hazard.duration = 0.0
                        import random
                        if random.random() < 0.5:
                            self.ball.stun_timer = max(getattr(self.ball, "stun_timer", 0.0), 3.0)
                            self.ball.speed = getattr(self.ball, "base_speed", 2.0) * 0.1
                        else:
                            new_trap = Hazard(len(self.world.arena.hazards), hazard.x, hazard.y, 30.0, "proximity_trap", 30.0)
                            self.world.arena.hazards.append(new_trap)
