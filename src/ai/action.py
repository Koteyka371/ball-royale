...
class Action:
    ...
    def execute(self, strategy: str, delta: float) -> None:
        ...
        if strategy in ("flee", "defend", "attack") and hasattr(self.ball, "inventory") and "hookshot" in self.ball.inventory:
            # Propel user to the closest wall or platform (entity) quickly for clutch saves
            ...
            if closest_target and closest_target_dist_sq < (closest_wall_dist ** 2):
                ...
                if dist > 0.0001:
                    move_dist = min(dist, pull_dist)
                    self.ball.x += ((closest_target.x - self.ball.x) / dist) * move_dist
                    self.ball.y += ((closest_target.y - self.ball.y) / dist) * move_dist
