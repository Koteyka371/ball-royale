        ...
        for b in list(balls):
            if getattr(b, "alive", False) and getattr(b, "ball_type", None) == "loot_goblin":
                ...
                if b.hp <= 0:
                    b.alive = False
                    if hasattr(world, "boosters"):
                        booster_kinds = ["damage_booster", "speed_booster", "charging_shockwave_shield_booster", "shield_booster", "hp_booster", "gravity_well_booster", "gravity_boots"]
                        for i in range(3):
    ...
    def tick(self, world: 'Any', balls: 'List[Any]', delta: float = 0.016) -> None:
        ...
        if self.cycle_timer <= 0:
            ...
            if hasattr(world, "arena") and hasattr(world.arena, "hazards"):
                ...
                self.rifts = []
                num_rifts = random.randint(3, 5)
                for i in range(num_rifts):
                    x = random.uniform(100, arena_w - 100)
                    y = random.uniform(100, arena_h - 100)
                    radius = random.uniform(40.0, 70.0)
                    kind = random.choice(["slow_motion_zone", "fast_motion_zone"])

                    h = Hazard(id=f"temporal_rift_{random.randint(1000, 99999)}", x=x, y=y, radius=radius, kind=kind, damage=0.0)
