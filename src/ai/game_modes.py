            ...
            if hasattr(world, "balls"):
                for minion in world.balls:
                    if getattr(minion, "ball_type", "") == "minion" and getattr(minion, "minion_owner", None) == getattr(ball, "id", None):
                        ...
                        b_dmg = getattr(minion, "base_damage", 10.0)
                        minion.base_speed = b_speed * 3.0
                        minion.base_damage = b_dmg * 2.5
                        minion.speed = getattr(minion, "speed", b_speed) * 3.0
                        minion.damage = getattr(minion, "damage", b_dmg) * 2.5
        ...
        for b in list(balls):
            if getattr(b, "alive", False) and getattr(b, "ball_type", None) == "loot_goblin":
                ...
                if b.hp <= 0:
                    b.alive = False
                    if hasattr(world, "boosters"):
                        booster_kinds = ["damage_booster", "speed_booster", "charging_shockwave_shield_booster", "shield_booster", "hp_booster"]
                        for i in range(3):
