"""
Auto-generated ball type: Hard
As described in game_design.md for 'Hard (Для профессионалов)' difficulty.
Fast reactions, optimal decisions, perfect skill usage, rarely mistakes.
"""

from ai.personality import Personality

class Hard:
    BALL_TYPE = "hard"
    HP = 150
    SPEED = 5.0
    DAMAGE = 20
    RADIUS = 12
    PERCEPTION_RADIUS = 300
    AGGRESSION = 0.8
    COLOR = "black"
    SKILL = "perfect_strike"
    SKILL_COOLDOWN = 4.0

    def __init__(self, ball_id: int, x: float = 0.0, y: float = 0.0):
        self.id = ball_id
        self.hp = float(self.HP)
        self.max_hp = float(self.HP)
        self.x = x
        self.y = y
        self.alive = True
        self.kills = 0
        self.first_hit_taken = False
        self.current_action = "idle"
        self.skill_timer = 0.0
        self.personality = Personality("aggressive")
        self.difficulty = "hard"

    def get_hp_percent(self) -> float:
        return self.hp / self.max_hp if self.max_hp > 0 else 0.0

    def flee(self, delta: float) -> None:
        self.current_action = "flee"

    def attack(self, delta: float) -> None:
        self.current_action = "attack"

    def defend(self, delta: float) -> None:
        self.current_action = "defend"

    def collect_booster(self, delta: float) -> None:
        self.current_action = "collect_booster"

    def idle(self, delta: float) -> None:
        self.current_action = "idle"

    def take_damage(self, amount: float) -> None:
        if self.hp == self.max_hp and amount > 0:
            self.first_hit_taken = True
        self.hp -= amount
        if self.hp <= 0:
            self.alive = False

    def use_skill(self) -> bool:
        if self.skill_timer <= 0:
            self.skill_timer = self.SKILL_COOLDOWN
            self.current_action = "attack"
            return True
        return False

    def kite(self, delta: float) -> None:
        self.current_action = "kite"

    def flank(self, delta: float) -> None:
        self.current_action = "flank"

    def group_attack(self, delta: float) -> None:
        self.current_action = "group_attack"

    def chase(self, delta: float) -> None:
        self.current_action = "chase"

    def __repr__(self) -> str:
        return f"{self.BALL_TYPE}#{self.id} HP={self.hp}/{self.max_hp} [{self.current_action}]"
