...
class BattleRoyaleMode extends GameMode:
	...
	func tick(world, balls: Array, delta: float = 0.016) -> void:
				...
				if tie_count == 1:
					if altar["owner"] == max_team:
						...
						if altar["capture_progress"] <= 0:
							...
							var pref = weather_prefs.get(max_team, "thunderstorm")
							var w = get_meta("weather") if has_meta("weather") else "clear"
							if w != pref:
								set_meta("weather", pref)
		...
		if controller != null:
			...
			var pref = weather_prefs.get(ctype, "thunderstorm")
