...
class ReverseTugOfWarMode extends GameMode:
	...
	func _init() -> void:
		...
		description = "Like Tug of War, but the payload moves AWAY from the team that has more players nearby. Teams must zone out enemies and avoid getting too close to push it towards the enemy goal."
	...
	func tick(world, balls: Array, delta: float = 0.016) -> void:
		...
		if payload != null:
			...
			if is_alive:
				...
				var current_px = payload.get("x", 0) if typeof(payload) == TYPE_DICTIONARY else payload.x
				if current_px < 50.0:
					if typeof(payload) == TYPE_DICTIONARY:
						payload["x"] = 50.0
					else:
						payload.x = 50.0
				elif current_px > arena_width - 50.0:
					if typeof(payload) == TYPE_DICTIONARY:
