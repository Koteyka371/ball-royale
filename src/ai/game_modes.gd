...
class DynamicWeatherMode extends BattleRoyaleMode:
	var weather_timer = 0.0
	var current_weather = "clear"
	var weather_types = ["clear", "wind", "storm", "rain"]
	var wind_dir_x = 1.0
	var wind_dir_y = 0.0
	...
	func _init() -> void:
		...
		description = "Weather changes periodically, affecting ball physics (e.g. wind pushing, storm dealing damage)."
	...
	func tick(world, balls: Array, delta: float = 0.016) -> void:
		...
		if weather_timer >= 5.0:
			...
			if current_weather == "wind" or current_weather == "storm":
		...
		for b in balls:
			if b.alive and b.ball_type != "spectator":
				if current_weather == "wind":
					# Wind pushes balls
					var wind_strength = 50.0
					b.x += wind_dir_x * wind_strength * delta
					b.y += wind_dir_y * wind_strength * delta
				elif current_weather == "storm":
					# Storm pushes randomly and occasionally deals damage
					b.x += wind_dir_x * 80.0 * delta
					b.y += wind_dir_y * 80.0 * delta
					if randf() < 0.01:
						if "hp" in b:
							b.hp -= 2.0
							if b.hp <= 0:
					...
					var angle = randf() * PI * 2.0
					b.x += cos(angle) * 15.0 * delta
					b.y += sin(angle) * 15.0 * delta
