...
class GuildVsGuildTournamentMode extends GameMode:
	var current_round = 1
	var max_rounds = 3
	var team_wins = {"Red": 0, "Blue": 0}
	var round_active = true
	var round_delay = 0.0
	var initial_positions = {}
	func _init() -> void:
		name = "Guild vs Guild Tournament"
		description = "Two clans face off in a multi-round tournament. Winning yields special clan cosmetics and bonus clan points."
	func setup(world, balls: Array) -> void:
		super.setup(world, balls)
		if not "dead_balls" in world:
			if world.has_method("set_meta"):
		...
		var valid_balls = []
		for b in balls:
			if b.get("ball_type") != "spectator" and b.get_meta("ball_type") != "spectator":
				valid_balls.append(b)
						var mid = valid_balls.size() / 2
		for i in range(valid_balls.size()):
			var b = valid_balls[i]
			if i < mid:
				...
				b.set("team", "Blue")
				if b.has_method("set_meta"):
					b.set_meta("team", "Blue")
			b.set("alive", true)
			var max_hp = b.get("max_hp") if b.get("max_hp") != null else 100.0
			b.set("hp", max_hp)
						var b_id = b.get("id") if b.get("id") != null else (b.get_meta("id") if b.has_method("get_meta") and b.has_meta("id") else null)
			if b_id != null:
				initial_positions[b_id] = {"x": b.get("x"), "y": b.get("y")}
	func tick(world, balls: Array, delta: float) -> void:
		super.tick(world, balls, delta)
				if not round_active:
			round_delay -= delta
			if round_delay <= 0:
				...
				for b in balls:
					...
					if typeof(b) == TYPE_DICTIONARY:
						...
						is_spec = b.get("ball_type") == "spectator" or (b.has_method("has_meta") and b.has_meta("ball_type") and b.get_meta("ball_type") == "spectator")
											if not is_spec:
						b.set("alive", true)
						var max_hp = b.get("max_hp") if b.get("max_hp") != null else 100.0
						b.set("hp", max_hp)
						var b_id = b.get("id") if b.get("id") != null else (b.get_meta("id") if b.has_method("get_meta") and b.has_meta("id") else null)
						if b_id != null and initial_positions.has(b_id):
							var pos = initial_positions[b_id]
							b.set("x", pos.x)
							b.set("y", pos.y)
			return
		var alive_red = false
		var alive_blue = false
				for b in balls:
			var team = ""
			var alive = false
						if typeof(b) == TYPE_DICTIONARY:
				...
				team = b.get("team") if b.get("team") != null else (b.get_meta("team") if b.has_method("has_meta") and b.has_meta("team") else "")
		...
		if round_winner != null:
			if team_wins.has(round_winner):
				team_wins[round_winner] += 1
			round_active = false
			round_delay = 3.0
	func check_winner(world, balls: Array):
		var target_wins = max_rounds / 2 + 1
		for team in team_wins.keys():
			if team_wins[team] >= target_wins:
				if "profile_manager" in world and world.profile_manager != null:
					if world.profile_manager.has_method("add_cosmetic"):
						world.profile_manager.add_cosmetic("Tournament Champion")
					if world.profile_manager.has_method("add_title"):
						world.profile_manager.add_title("Clan Legend")
					if "leaderboard_manager" in world.profile_manager and world.profile_manager.leaderboard_manager != null:
						var lm = world.profile_manager.leaderboard_manager
						if "data" in lm:
							if not lm.data.has("clan_points"):
								lm.data["clan_points"] = {}
							lm.data["clan_points"][team] = lm.data["clan_points"].get(team, 0) + 100
							if lm.has_method("save"):
								lm.save()
				return team
		return null
func _ready():
	GAME_MODES["gvg_tournament"] = GuildVsGuildTournamentMode.new()
