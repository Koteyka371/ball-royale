...
class PlayerVsNeuralMode extends GameMode:
	func _init() -> void:
		name = "Player vs Neural"
		description = "Players fight against learning neural networks to shape their strategy."
	...
	func setup(world, balls: Array) -> void:
		...
		var players_count = 0
		if valid_balls.size() > 0:
			players_count = min(1, valid_balls.size() / 2)
		if players_count == 0 and valid_balls.size() > 1:
			players_count = 1
...
var GAME_MODES = {
	...
	"player_vs_neural": PlayerVsNeuralMode.new()
}
