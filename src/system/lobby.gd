...
func select_trap_variant(ball_id: int, variant: String) -> void:
    if variant in ["normal", "poison", "stun", "ricochet", "emp", "hologram", "chain_lightning", "decoy", "blindness", "clone", "tar"]:
...
func apply_random_loadout(ball_id: int, profile: ProfileManager) -> bool:
    ...
    var trap_variants = ["normal", "poison", "stun", "ricochet", "emp", "hologram", "chain_lightning", "decoy", "mine", "warp", "siphon", "clone", "tar"]
