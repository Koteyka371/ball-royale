import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from ai.action import Action

class MockBall:
    def __init__(self, id=1, x=0, y=0):
        self.id = id
        self.hp = 100
        self.max_hp = 100
        self.alive = True
        self.x = x
        self.y = y
        self.vx = 0
        self.vy = 0
        self.team = "player"
        self.ball_type = "basic"
        self.speed = 100
        self.base_speed = 100
        self.radius = 15.0
        self.skill = None
        self.active_skill = None
        self.damage = 10
        self.intangible = False
        self.intangible_timer = 0.0
        self.speed_multiplier = 1.0
        self.slow_timer = 0.0
        self.can_jump = True
        self.is_dashing = True

class MockHazard:
    def __init__(self, owner_id):
        self.id = 100
        self.kind = "trap"
        self.trap_variant = "web"
        self.owner_id = owner_id
        self.x = 50
        self.y = 50
        self.radius = 15.0
        self.damage = 10.0
        self.duration = 5.0
        self.active = True

class MockArena:
    def __init__(self):
        self.hazards = []
        self.width = 1000
        self.height = 1000
    def clamp_position(self, x, y, radius):
        return x, y, False
    def update_zone(self, tick, delta):
        pass

class MockWorld:
    def __init__(self):
        self.balls = []
        self.arena = MockArena()
        self.tick = 0
        self.next_id = 9999
        self.events = []
    def _deal_damage(self, attacker, target, damage=None):
        dmg = damage if damage is not None else attacker.damage
        target.hp -= dmg
    def add_event(self, event_type, data):
        self.events.append((event_type, data))

def test_spiderweb_trap_trigger():
    owner = MockBall(id=2, x=200, y=200)
    triggering_ball = MockBall(id=1, x=45, y=45)

    world = MockWorld()
    hazard = MockHazard(owner_id=owner.id)
    world.arena.hazards.append(hazard)
    world.balls = [triggering_ball, owner]

    action = Action(triggering_ball.id, world)
    action.ball = triggering_ball

    action.execute("idle", 0.1)

    assert hazard.duration == 0.0

    spider_webs = [h for h in world.arena.hazards if getattr(h, "kind", "") == "spider_web"]
    assert len(spider_webs) == 1

    web = spider_webs[0]
    assert web.x == 50
    assert web.y == 50
    assert getattr(web, "duration", 0.0) == 8.0

    assert triggering_ball.speed_multiplier == 0.2
    assert triggering_ball.slow_timer == 3.0

    assert getattr(triggering_ball, 'is_dashing', True) is False
    assert getattr(triggering_ball, 'can_jump', True) is False

def test_spiderweb_effect():
    enemy = MockBall(id=1, x=50, y=50)

    world = MockWorld()

    web = type('MockHazard', (), {})()
    web.id = 101
    web.kind = "spider_web"
    web.x = 50
    web.y = 50
    web.radius = 40.0
    web.damage = 0.0
    web.duration = 10.0
    web.active = True

    world.arena.hazards.append(web)
    world.balls = [enemy]

    action = Action(enemy.id, world)
    action.ball = enemy

    action.execute("idle", 0.1)

    assert enemy.speed_multiplier <= 0.5
    assert enemy.slow_timer >= 2.0

    assert getattr(enemy, 'is_dashing', True) is False
    assert getattr(enemy, 'can_jump', True) is False
