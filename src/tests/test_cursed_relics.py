...
def test_cursed_relics_mode():
    ...
    assert balls[0].damage == balls[0].base_damage * 2.0 # double damage
    assert balls[0].base_damage_cursed == getattr(balls[0], "base_damage_cursed", 10.0)
    ...
    mode.tick(world, balls, 0.016)
    assert balls[0].damage == getattr(balls[0], "base_damage", 10.0)
    assert not hasattr(balls[0], "base_damage_cursed")
    assert balls[1].damage == getattr(balls[1], "base_damage", 10.0) * 2.0
