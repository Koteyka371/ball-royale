...
def test_web_trap_trigger():
    ...
    # Spider web should be spawned
    spider_webs = [h for h in world.arena.hazards if getattr(h, "kind", "") == "spider_web"]
    assert len(spider_webs) == 1

    web = spider_webs[0]
    assert web.x == 50
    assert web.y == 50
    assert getattr(web, "duration", 0.0) == 10.0

    # Triggering ball should be heavily slowed
    assert triggering_ball.speed_multiplier == 0.3
    assert triggering_ball.slow_timer == 4.0

def test_spider_web_effect():
    ...
    world.arena.hazards.append(web)
    world.balls = [enemy]

    action = Action(enemy.id, world)
    action.ball = enemy

    # Effect is applied passively
    action.execute("idle", 0.1)

    assert enemy.speed_multiplier <= 0.3
