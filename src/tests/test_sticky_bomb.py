...
def test_sticky_bomb_attaches():
    world = MockWorld()
    world.next_id = 9999
    b1 = MockBall(0, 0, 1)
    world.balls.append(b1)

    bomb = MockHazard(10, 0, "sticky_bomb")
...
def test_sticky_bomb_transfers():
    world = MockWorld()
    world.next_id = 9999
    b1 = MockBall(0, 0, 1)
...
def test_sticky_bomb_explodes():
    world = MockWorld()
    world.next_id = 9999
    b1 = MockBall(0, 0, 1)
    world.balls.append(b1)

    bomb = MockHazard(0, 0, "sticky_bomb")
    bomb.attached_id = 1
    bomb.duration = 0.05
    world.arena.hazards.append(bomb)
...
def test_sticky_bomb_booster_collect():
    world = MockWorld()
    world.next_id = 9999
    b1 = MockBall(0, 0, 1)
    world.balls.append(b1)

    booster = MockBooster(5, 5, "sticky_bomb_booster")
    world.boosters.append(booster)

    # Mock world method so Action doesn't fail finding items
    ...
    assert world.arena.hazards[0].kind == "sticky_bomb"
