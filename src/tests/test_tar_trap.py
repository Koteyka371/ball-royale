...
def test_tar_trap_trigger():
    ...
    # Tar puddle should be spawned
    tar_puddles = [h for h in world.arena.hazards if getattr(h, "kind", "") == "tar_puddle"]
    assert len(tar_puddles) == 1

    puddle = tar_puddles[0]
    assert puddle.x == 50
    assert puddle.y == 50
    assert getattr(puddle, "duration", 0.0) == 10.0

    # Triggering ball should be heavily slowed
    assert triggering_ball.speed_multiplier == 0.2
    assert triggering_ball.slow_timer == 5.0

def test_tar_puddle_effect():
    ...
    assert enemy.speed_multiplier <= 0.2
