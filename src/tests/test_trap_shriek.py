...
def test_shriek_trap_trigger():
    ...
    assert getattr(ball, "perception_radius") == 25.0  # 250 * 0.1
...
def test_shriek_trap_stealth_reveal():
    ball = MockBall()
    ball.invisible_timer = 5.0
    ball.is_invisible = True
    world = MockWorld()
    world.balls = [ball]
    ...
    assert getattr(ball, "invisible_timer", 0.0) == 0.0
    assert getattr(ball, "is_invisible", True) == False
